# -*- coding: utf-8 -*-
"""
СКАНЕР ВЛИВАНИЙ v8 — ЛОНГ + ТРЕУГОЛЬНИК + РАЗВОРОТ + FOLLOW-UP + /stats2
=========================================================================
Telegram-бот для Railway / VPS. Бот НЕ торгует сам. Он:
1) ищет лонг-сетапы (OI↑ + объём↑ + тренд вверх);
2) шлёт карточку треугольника при triangle ready/breakout;
3) шлёт карточку РАЗВОРОТА после сползания + объёмного разворота (с
   подтверждением следующей свечой, чтобы не ловить ложный спайк);
4) ведёт позицию по кнопке «Я вошёл»;
5) логирует каждый сигнал в SIGNALS_FILE;
6) считает форвардную статистику по сигналам через /stats и /stats2.

v8 = v7 + аудит и фиксы:
- EARLY_RSI_MAX теперь реально используется в early_breakout();
- FUNDING_CUTOFF используется вместо хардкода 0.01 в _score()/card_long();
- удалены неиспользуемые CALM_UPDATE_MIN и QUOTE;
- удалены дубли pos_buttons()/close_trade()/position_status();
- удалён мёртвый фильтр THIN_TURN (universe() уже фильтрует по обороту);
- фильтр "3 зелёные свечи" применяется теперь ко ВСЕМ типам сигналов
  (long, long_fast, triangle, early) — единая логика подтверждения;
- новый тип сигнала РАЗВОРОТ (reversal_setup) с подтверждением через
  закрытие следующей свечи выше триггера (reversal_pending в WATCH);
- проверка спреда bid/ask через тикер (spread_ok) перед отправкой любой
  карточки — отсекает сигналы на тонком стакане.
"""
...
# новые константы v8 (разворот)
REVERSAL_COOLDOWN_H = 4
REVERSAL_VOL_MIN = 1.8
REVERSAL_OI_MIN = 0.02
REVERSAL_LOOKBACK_DOWN = 10
REVERSAL_NO_LOW_BARS = 3
REVERSAL_NO_LOW_BASE = 8

# новые константы v8 (спред)
MAX_SPREAD_PCT = 0.0025  # 0.25% — сигнал считается риском узкой ликвидности выше этого
...
def three_green_30m(symbol):
    ...
    Применяется как единый фильтр подтверждения для ВСЕХ типов сигналов
    (long, long_fast, triangle, early, reversal) через флаг REQUIRE_3_GREEN_30M.
    """
    try:
...
def green3_ok(symbol):
    """Единая точка входа для фильтра '3 зелёные свечи' — применяется ко всем сигналам."""
    if not REQUIRE_3_GREEN_30M:
        return True
    return three_green_30m(symbol)
...
def ticker_info(symbol):
    """
    Возвращает цену, funding, оборот и bid/ask спред монеты.
    Спред считается из bid1Price/ask1Price того же тикера — без
    дополнительных запросов к API (данные уже в кэше all_tickers()).
    """
...
def spread_ok(ex):
    """True если спред приемлемый (или данные недоступны — fail-open)."""
    if not ex:
        return True
    return ex.get("spread_pct", 0) <= MAX_SPREAD_PCT


def orderbook_depth(symbol, limit=25):
    """
    Доп. проверка глубины стакана (вызывать только для монет, УЖЕ прошедших
    остальные фильтры — у orderbook endpoint отдельный, более строгий rate limit).
    """
    try:
        res = bget("/v5/market/orderbook", {"category": "linear", "symbol": symbol, "limit": limit})
        bids = res.get("b") or []
        asks = res.get("a") or []
        ...
        bid_depth_usd = sum(float(p) * float(s) for p, s in bids[:10])
        ask_depth_usd = sum(float(p) * float(s) for p, s in asks[:10])
        return dict(spread_pct=spread_pct, bid_depth=bid_depth_usd, ask_depth=ask_depth_usd)
    except Exception:
        return None
...
def detect_triangle(highs, lows, closes, price, win=45, swing_win=3, min_touches=MIN_TRIANGLE_TOUCHES):
    n = len(closes)
    if n < win + 5:
...
def early_breakout(closes, highs, lows, vols, rsi_val=None):
    """
    v8: теперь принимает rsi_val и реально проверяет его против EARLY_RSI_MAX —
    раньше константа была задана, но не использовалась.
    """
    if len(closes) < 40:
    ...
    rsi_ok = (rsi_val is None) or (rsi_val <= EARLY_RSI_MAX)
    return (compressed and broke_up and vol_ok and rsi_ok), zone_hi, zone_lo


def reversal_setup(closes, highs, lows, vols, oic):
    """
    v8 НОВОЕ: ловит паттерн "сползание -> затухание падения -> всплеск
    объёма на зелёной свече -> рост OI (отличает от шорт-сквиза)".
    Возвращает (ok, details) — details используется в карточке и для
    отложенного подтверждения через reversal_confirmed().
    """
    if len(closes) < 70 or len(oic) < 4:
        return False, {}

    e50_now = ema(closes[-60:], 50)
    ...
    vol_spike = vol_avg > 0 and vols[-1] >= vol_avg * REVERSAL_VOL_MIN
    ...
    details = dict(
        ...
        prior_low=prior_low,
        vol_ratio=(vols[-1] / vol_avg if vol_avg > 0 else 0),
        oi_chg=oi_chg,
    )
    return ok, details


def reversal_confirmed(closes, entry_price):
    """
    Против ложного спайка: цена должна ЗАКРЫТЬСЯ выше уровня входа хотя бы
    на следующей свече. Вызывается из check_watchlist() для kind=='reversal_pending'.
    """
    if not closes:
        return False
    return closes[-1] > entry_price
...
def core(coin, closes, highs, lows, vols, oic, btc, btc_p4=0.0, tri_mtf=None):
    ...
    return dict(
        coin=coin, price=price, p1=p1, p4=p4, oi1=oi1, oi4=oi4, oi24=oi24,
        spike=spike, spike_1h=spike_1h, uptrend=uptrend, dd=dd, turn=turn,
        cor=cor, tf=tf, brk=brk, rsi=r, btc_beta=btc_beta, e21=e21, ext=ext,
        consol_base=consol_base, old_high=old_high, extended=extended,
...
def long_ok(m):
    """v8: убрана мёртвая проверка m['turn'] >= THIN_TURN — universe() уже
    гарантирует оборот >= MIN_TURNOVER_24H (30М), THIN_TURN (5М) никогда
    не мог провалиться."""
    return (
        m["oi4"] >= OI_4H_MIN
...
def long_ok_fast(m):
    """
    РАННИЙ триггер лонга по 1-часовым данным. Не ждёт полного 4-часового
    накопления OI/объёма/цены как long_ok() — ловит начало движения раньше.
    """
    return (
...
def _score(m, ex):
    """v8: FUNDING_CUTOFF вместо хардкода 0.01."""
    s = 0
    ...
    if ex.get("funding", 0) > FUNDING_CUTOFF:
        s -= 1
...
def track_record(sig_type):
    if time.time() - _track_cache["ts"] < 600 and sig_type in _track_cache["data"]:
...
def card_long(m, ex):
    ...
    if ex.get("spread_pct", 0) > MAX_SPREAD_PCT:
        cautions.append(f"широкий спред ({ex.get('spread_pct',0)*100:.2f}%) — тонкий стакан")
    sc = _score(m, ex)
...
def card_reversal(m, ex, details):
    """v8 НОВОЕ: карточка сигнала РАЗВОРОТ (после сползания + всплеска объёма)."""
    sc = _score(m, ex)
    rsi_v = int(m.get("rsi", 50))
    lines = [
        f"🔻→🚀 {m['coin']} · РАЗВОРОТ (объём после сползания)",
        f"💵 ${m['price']:.5g} (Bybit)",
        "",
        f"💪 Сила сетапа: {sc}/10 {_bar(sc/10,5)}",
        f"📉 Объём ×{details.get('vol_ratio',0):.1f} от нормы · OI {details.get('oi_chg',0)*100:+.1f}% · RSI {rsi_v}",
        f"🔗 Корреляция с BTC {m['cor']*100:.0f}%",
        "",
        "⚠️ Разворот после даунтренда — риск шорт-сквиза выше, чем у обычного лонга.",
        "• подтверждён рост OI — это новые деньги, не закрытие шортов",
        "• сигнал уже прошёл проверку закрытием следующей свечи выше триггера",
        f"• стоп под минимумом разворота: ${details.get('recent_low', m['price']):.5g}",
    ]
    _n, _w = track_record("reversal")
    if _n > 0:
        lines += ["", f"📈 Трек-рекорд РАЗВОРОТОВ: измерено {_n}, в плюсе через 24ч {_w} ({_w/_n*100:.0f}%)"]
    lines += ["", "━━━━━━━━━━━━━━━━", "⚠️ Подсветка, не приказ. Стоп на Bybit обязателен."]
    return "\n".join(lines)
...
def compute_stats():
    ...
    for horizon_h, label in [(4, "4ч"), (24, "24ч")]:
        for sig_type in ("long", "long_fast", "triangle", "early", "reversal"):
            sig_pcts = []
            btc_pcts = []
            for r in rows:
                if r["type"] != sig_type:
...
# ---------------------------------------------------------------------------
# Position follow-up (v8: дубли удалены, оставлено единственное определение)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Watchlist (retest зон + подтверждение разворота через след. свечу)
# ---------------------------------------------------------------------------
def check_watchlist(chat):
    ...
    for coin in list(WATCH):
        ...
        # v8 НОВОЕ: подтверждение сигнала РАЗВОРОТ следующей свечой
        if w.get("kind") == "reversal_pending":
            ...
            if len(k) < 1:
                continue
            last_closed = k[1] if len(k) > 1 else k[0]
            ...
            if close_price > w["trigger_price"]:
                ex = ticker_info(w["sym"])
                if ex and not spread_ok(ex):
                    del WATCH[coin]
                    continue
                m = w["m_snapshot"]
                m["price"] = close_price
                card = card_reversal(m, w["ex_snapshot"], w["details"])
                tg_send(chat, card, buttons=pos_buttons(coin))
                log_signal(coin, "reversal", close_price)
                del WATCH[coin]
            continue

        # существующая retest-логика треугольников/уровней
...
def run_scan(cid, announce=False):
    ...
    btc_hits, btc_reasons, btc_price_now = btc_short_risk()
    btc_block = btc_hits >= BTC_RISK_MIN_HITS
    if btc_block:
        now_ts = time.time()
        if now_ts - LAST_BTC_WARN > 3600:
            tg_send(cid, "🛑 BTC-РУБИЛЬНИК: " + "; ".join(btc_reasons) + "\nНовые лонг-сигналы приостановлены.")
            LAST_BTC_WARN = now_ts
            try:
                new = not os.path.exists(BLOCKS_FILE)
                with open(BLOCKS_FILE, "a", newline="") as f:
                    w = csv.writer(f)
                    if new:
                        w.writerow(["ts", "btc_price", "reasons"])
                    w.writerow([dt.datetime.now().isoformat(timespec="seconds"), f"{btc_price_now or 0:.2f}", "; ".join(btc_reasons)])
            except Exception:
                pass

    btc_p4 = btc_closes[-1] / btc_closes[-5] - 1 if len(btc_closes) >= 5 else 0.0
    now = time.time()
    alerts_sent = 0
    ...
    for coin, sym in coins:
        ...
        SYM_CACHE[coin] = sym
        try:
            closes, highs, lows, vols = klines(sym, limit=MIN_BARS)
            time.sleep(0.10)
            oic = open_interest(sym, limit=30)
            time.sleep(0.10)
        except Exception:
            continue
        if len(closes) < 70 or len(oic) < 6:
            continue

        age = listing_age_days(sym)
        if age is not None and age < MIN_LISTING_AGE_DAYS:
            continue

        ex = ticker_info(sym)
        if not ex or not spread_ok(ex):
            continue

        m = core(coin, closes, highs, lows, vols, oic, btc_closes, btc_p4=btc_p4)
        m["bybit"] = ex.get("price")

        loss_ts = RECENT_LOSSES.get(coin)
        in_loss_cooldown = loss_ts and (now - loss_ts < COOLDOWN_H * LOSS_COOLDOWN_MULT * 3600)

        # 🟢 ЛОНГ (подтверждённый, 4ч)
        if not btc_block and not in_loss_cooldown and long_ok(m) and now - LAST_ALERT.get(coin, 0) > COOLDOWN_H * 3600:
            if green3_ok(sym):
                card = card_long(m, ex)
                tg_send(cid, card, buttons=pos_buttons(coin))
                log_signal(coin, "long", m["price"])
                LAST_ALERT[coin] = now
                alerts_sent += 1
                continue

        # 🟠 РАННИЙ ЛОНГ (1ч-триггер) — только если полный long_ok ещё не сработал
        if not btc_block and not in_loss_cooldown and not long_ok(m) and long_ok_fast(m) and now - LAST_ALERT.get(coin, 0) > FAST_SIGNAL_COOLDOWN_H * 3600:
            if green3_ok(sym):
                card = card_long_fast(m, ex)
                tg_send(cid, card, buttons=pos_buttons(coin))
                log_signal(coin, "long_fast", m["price"])
                LAST_ALERT[coin] = now
                alerts_sent += 1
                continue
        ...
        # 🔺 ТРЕУГОЛЬНИК
        tri_now = m.get("tri")
        if tri_now in ("ready", "breakout") and now - TRI_ALERT.get(coin, 0) > TRI_ALERT_HOURS * 3600:
            if green3_ok(sym):
                tri_mtf = detect_triangle_mtf(sym)
                m["tri_mtf"] = tri_mtf
                if tri_mtf.get("15м") in ("ready", "breakout"):
                    card = card_triangle(m, ex)
                    if card:
                        tg_send(cid, card, buttons=pos_buttons(coin))
                        log_signal(coin, "triangle", m["price"])
                        TRI_ALERT[coin] = now
                        alerts_sent += 1
                        continue

        # 🔵 РАННИЙ (сжатие) — EARLY_RSI_MAX теперь реально проверяется
        if EARLY_ENABLED and now - LAST_EARLY.get(coin, 0) > EARLY_COOLDOWN_H * 3600:
            early, zhi, zlo = early_breakout(closes, highs, lows, vols, rsi_val=m.get("rsi"))
            if early and m.get("atrr", 1.0) >= ATR_MIN_RATIO and green3_ok(sym):
                card = card_early(m, zhi, zlo)
                tg_send(cid, card, buttons=pos_buttons(coin))
        ...
        # 🔻→🚀 РАЗВОРОТ — не шлём сразу, ставим на подтверждение след. свечой
        ok, details = reversal_setup(closes, highs, lows, vols, oic)
        if ok and now - LAST_REVERSAL.get(coin, 0) > REVERSAL_COOLDOWN_H * 3600 and green3_ok(sym):
            WATCH[coin] = dict(
                ts=now,
                sym=sym,
                kind="reversal_pending",
                trigger_price=m["price"],
                stop_price=details.get("recent_low", m["price"]),
                details=details,
                m_snapshot=dict(m),
                ex_snapshot=dict(ex),
            )
...
def main():
    ...
    while True:
            ...
            for u in upd.get("result", []):
                ...
                if "message" in u:
                    ...
                    if text == "/start":
                        chat = cid
                        save_chat(chat)
                        tg_send(cid, "✅ Бот запущен. Команды: /scan /stats /stats2 /btcblocks")
                    elif text == "/scan":
        ...
        now = time.time()
        if chat and now - last_scan > SCAN_EVERY_MIN * 60:
            try:
                run_scan(chat)
            except Exception as e:
                print("scan:", e)
        ...
        if chat and POSITIONS and now - last_pos_check > CHECK_POS_MIN * 60:
            for coin in list(POSITIONS):
                msg, state = position_status(coin)
                if msg:
                    tg_send(chat, msg)
            last_pos_check = now

        time.sleep(WATCH_CHECK_SEC)
